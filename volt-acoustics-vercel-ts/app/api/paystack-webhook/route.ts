...
export async function GET() {
  return NextResponse.json({ ok: true, message: "Paystack webhook endpoint is live" });
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (payload.event === "charge.success") {
      ...
      } catch (e) { console.error("Verify call failed", e); }

      // Optional: persist in Supabase
      ...
      if (supabaseUrl && supabaseKey) {
        ...
        const { error } = await supabase.from("orders").insert({
          reference,
          status: verified?.data?.status || payload?.data?.status || "unknown",
          amount,
          email,
          raw_payload: payload,
          verified_payload: verified ?? null,
        });
